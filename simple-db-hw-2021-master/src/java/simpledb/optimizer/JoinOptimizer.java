...
public class JoinOptimizer {
            ...
            double cost1, double cost2)
    {
        ...
        }//t1的IO成本+  t1记录数*t2的IO成本  + t1记录数*t2记录数(cpu成本)
        //对于连接查询来说，以两表连接为例，首先需要扫描一张表然后过滤出一些记录，然后把过滤完的记录，每一条都去与第二张表进行匹配，这里第一张表称为驱动表t1，第二张表称为被驱动表t2。
        // 在两表连接中，驱动表只需要扫描一次，然后产生card1条记录，而被驱动表则需要扫描card1次，这是总的IO成本；然后假设t2表有card2条记录，则产生的CPU成本应该为card1 * card2。所有总成本应该为
    }
     ...
     */
    public <T> Set<Set<T>> enumerateSubsets(List<T> v, int size) {//返回列表v的大小为size的子集,这是一个n！复杂度的算法，不佳
    ...
    public <T> HashMap<Integer, Set<Set<T>>> buildMap(List<T> v){
        final HashMap<Integer , Set<Set<T>>> result = new HashMap<>();//<元素数量，集合的集合>
        int n=v.size();//v有n个元素
        int state = 0;
        while(state<(1<<n)){//1<<n表示v有2的n次个子集
            Set<T> cur = new HashSet<>();
            for(int j=0; j<n;j++){
                if(((state>>j) & 1) == 1)//实现的是判断当前state的二进制数的第j位是否为1 ， 把那些为1的位对应的元素都加入（老鼠毒药问题）
                    cur.add(v.get(j));
            }
            int count = Integer.bitCount(state);//计算二进制数有多少个1
            if(result.containsKey(count))//如果这一种数量已经有了，就把cur集合作为’集合的集合‘的元素加入
    ...
    public List<LogicalJoinNode> orderJoins(
        ...
        HashMap<Integer , Set<Set<LogicalJoinNode>>> sizeToJoins = buildMap(joins);
        for(int i = 1;i<=n;i++){
            Set<Set<LogicalJoinNode>> sets = sizeToJoins.get(i);//遍历1个的开始
            for(Set<LogicalJoinNode> subset : sets){
                double bestCost = Double.MAX_VALUE;
                for(LogicalJoinNode removeNode : subset){
                    final CostCard cc = computeCostAndCardOfSubplan(stats,filterSelectivities,removeNode,subset,bestCost,pc);
                    if(cc != null){
                        bestCost =cc.cost;
                        costCard =cc;
                    }
    ...
    private CostCard computeCostAndCardOfSubplan(
        ...
        //给出连接的子集joinSet，以及需要从集合中移除的joinToRemove，该方法计算将joinToRemove加入到joinSet-{joinToRemove}的最佳排序方式。
        // 它返回CostCard对象，该对象包含成本、基数和最佳的连接顺序(以列表形式返回)。
        ...
        int t1card, t2card;
        boolean leftPkey, rightPkey;//记录是否为主键，关系到card估算

        if (news.isEmpty()) { // base case -- both are base relations
            //如果 news 是空的，表示 joinSet 中只有一个元素 j，那么 j 是基本关系。在这种情况下，将 prevBest 初始化为空列表
            // 并计算表 table1Alias 和 table2Alias 的扫描成本和表基数。同时，判断是否为主键。
            ...
            t2cost = table2Alias == null ? 0 : stats.get(table2Name)//table2是可以空的，比如外连接的时候
                    .estimateScanCost();
            ...
            rightPkey = table2Alias != null && isPkey(table2Alias,
                    j.f2PureName);
        } else {   // news is not empty -- figure best way to join j to news
            //如果 news 不为空，表示 joinSet 中有多个元素，需要找到将 j 连接到 news 的最佳方式。
            // 首先，通过调用 pc.getOrder(news) 获取 news 的最佳连接顺序，并将结果存储在 prevBest 中
            // 如果 prevBest 为 null，表示没有缓存的答案，可能是因为子集包含了一个交叉连接。在这种情况下，返回 null
            ...
            if (doesJoin(prevBest, table1Alias)) { // j.t1 is in prevBest
                //根据连接顺序中的左表确定右子树的成本估计。如果连接顺序中包含 table1Alias，则将左子树的成本和基数设置为 prevBest 的成本和基数
                //同时，计算 table2Alias 的扫描成本和表基数，并判断右表是否有主键。
                ...
                rightPkey = j.t2Alias != null && isPkey(j.t2Alias, j.f2PureName);
                                                        ...
                                                        // (both shouldn't be)
                //如果连接顺序中包含 j.t2Alias，则将右子树的成本和基数设置为 prevBest 的成本和基数。同时，计算 table1Alias 的扫描成本和表基数，并判断左表是否有主键。
        ...
        cc.plan.add(j); // prevbest is left tree-- add new join to end
        return cc;
    }
