...
public class BTreeInternalPage extends BTreePage {
	...
	public BTreeInternalPage(BTreePageId id, byte[] data, int key) throws IOException {
		...
		try{
			...
			for (int i=1; i<keys.length; i++)
				keys[i] = readNextKey(dis,i);//key的index从1到m
	...
	private void deleteEntry(BTreeEntry e, boolean deleteRightChild) throws DbException {
		RecordId rid = e.getRecordId();//The recordId is used to find the specified entry
		if(rid == null)
		...
		//ATTENTION： deleteRightChild is true, delete the right child.
		//  Otherwise delete the left child
		if(deleteRightChild) {
			markSlotUsed(rid.getTupleNumber(), false);//隐式地删除了keys[i]和children[i]
		}
		else {//删左child
			for(int i = rid.getTupleNumber() - 1; i >= 0; i--) {
				if(isSlotUsed(i)) {//从rid.getTupleNumber出发找到第一个非空的槽，其实就是找keys[rid.getTupleNumber]的左子指针
	 ...
	 */
	//最粗略理解：
	// 假设我们有一个BTreeInternalPage对象，其中包含以下信息：
	//键值：[10, 20, 30]
	//子节点：[Child1, Child2, Child3, Child4]
	//现在，我们要在该BTreeInternalPage对象中插入一个新的BTreeEntry对象，该对象具有以下属性：
	//键值：25  左子节点：ChildA  右子节点：ChildB
	//在插入操作的过程中，会遍历该BTreeInternalPage节点的键值和子节点，找到合适的位置插入新的BTreeEntry。
	//插入后，该BTreeInternalPage对象的children数组将变为：[Child1, Child2, ChildA, ChildB, Child3, Child4]
	public void insertEntry(BTreeEntry e) throws DbException {
		...
		if(getNumEmptySlots() == getMaxEntries()) {
			...
			markSlotUsed(0, true);
			markSlotUsed(1, true);
			e.setRecordId(new RecordId(pid, 1));//tupleno和keys的索引一致
			return;
		}

		// find the first empty slot, starting from 1
		int emptySlot = -1;//后续移动时候要用
		...
		for (int i=0; i<numSlots; i++) {
			if(isSlotUsed(i)) {
				//children[i]对应于keys[i]的右子指针（以后说成children[i]是keys[i]的右子指针）
				if(children[i] == e.getLeftChild().getPageNumber() || children[i] == e.getRightChild().getPageNumber()) {
				//keys[i]右子指针等于e的左/右子指针，e还比keys[i]小，这是不可能的
					if(i > 0 && keys[i].compare(Op.GREATER_THAN, e.getKey())) {
						//keys[i]的键值比插入e的键值大
						//而他们的left child或right child又一致，这不可能！
					...
					//避免了if情况后，keys[lessOrEqKey]是“小于等于e.getkey()”的key中最大的那个，插入e后无论如何，e的key在keys[lessOrEqKey]右侧一个
					// 明确这一点，再结合children[i]是keys[i]的右child
					// 得到：e的左child将做keys[lessOrEqKey]的右child，e的右child的索引将和e的key索引保持一致
					lessOrEqKey = i;
				...
				//
					...
					if(keys[i].compare(Op.LESS_THAN, e.getKey())) {//检查下一个key是否大于等于e的key，若不成立，报错
						throw new DbException("attempt to insert invalid entry with left child " + 
								e.getLeftChild().getPageNumber() + ", right child " +
								e.getRightChild().getPageNumber() + " and key " + e.getKey() +
								" HINT: one of these children must match an existing child on the page" +
		...
		int goodSlot = -1;
		//keys[lessOrEqKey]是“小于等于e.getkey()”的key中最大的那个
		if(emptySlot < lessOrEqKey) {//空的在左边，从空到待插入全部左移
			...
			for(int i = emptySlot; i > lessOrEqKey + 1; i--) {//空的在右边，从空到待插入全部右移
				moveEntry(i-1, i);
			}
			goodSlot = lessOrEqKey + 1;//若右移，keys[lessOrEqKey]显然不配再向右移动，否则它比e.getKey()小却出现在了右边
		}
