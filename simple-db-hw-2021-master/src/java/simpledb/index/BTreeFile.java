...
public class BTreeFile implements DbFile {
    ...
    //* If f is null, it finds the left-most leaf page -- used for the iterator
        ...
        while(iterator.hasNext()){//遍历entry，直到找到一个entry的key小于等于f
            next = iterator.next();
    ...
    public BTreeLeafPage splitLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
        ...
        Tuple upper = iterator.next();
        BTreeInternalPage parentPage = getParentWithEmptySlots(tid,dirtypages,page.getParentId(),field);
        //调用这个函数，意味着：1.parentPage是还有空槽的 2.parentPage若非根结点，没什么特殊的 ，若为根节点，它有一个子结点为老根节点，等下加一个新的子结点
        BTreeEntry insertEntry = new BTreeEntry(upper.getField(keyField),page.getId(),newPage.getId());
        ...
        if(field.compare(Op.GREATER_THAN_OR_EQ,upper.getField(keyField)))
            return newPage;
        return page;
    }
    ...
    public BTreeInternalPage splitInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
        ...
        parent.insertEntry(pushEntry);//我只是把entry放入了，还没把parent弄好
        updateParentPointers(tid,dirtypages,page);//通过page里的entry，找到entry对应的child，让child的parent改掉
...
//如果父节点是根节点
        if(parentId.pgcateg() == BTreePageId.ROOT_PTR) {
            ...
            BTreeRootPtrPage rootPtr = (BTreeRootPtrPage) getPage(tid, dirtypages, // 根指针
                    BTreeRootPtrPage.getId(tableid), Permissions.READ_WRITE);
            BTreePageId prevRootId = rootPtr.getRootId(); //prevRootId存老根
            rootPtr.setRootId(parent.getId());//更新根
    ...
    private void updateParentPointer(TransactionId tid, Map<PageId, Page> dirtypages, BTreePageId pid, BTreePageId child)
            throws DbException, TransactionAbortedException {
   //函数修改child页的parent为pid
        BTreePage p = (BTreePage) getPage(tid, dirtypages, child, Permissions.READ_ONLY);//取出child

        if(!p.getParentId().equals(pid)) {//child的parent不是pid
            p = (BTreePage) getPage(tid, dirtypages, child, Permissions.READ_WRITE);//修改Permissions
            p.setParentId(pid);//设置parent
        }
    ...
    private void updateParentPointers(TransactionId tid, Map<PageId, Page> dirtypages, BTreeInternalPage page)
            throws DbException, TransactionAbortedException{
        //把page设置为page里所有entry e对应的child的parent
    ...
    Page getPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreePageId pid, Permissions perm)
            throws DbException, TransactionAbortedException {
        if(dirtypages.containsKey(pid)) {//dirtypages是本地缓存，若里面有就从这里拿
            return dirtypages.get(pid);
        } else {
            Page p = Database.getBufferPool().getPage(tid, pid, perm);//没有就去缓存池拿
            if(perm == Permissions.READ_WRITE) {//如果是写锁，加入dirtypages
    ...
    public List<Page> insertTuple(TransactionId tid, Tuple t)
        ...
        return new ArrayList<>(dirtypages.values());//return a list of all pages that were dirtied by this operation
    }
    ...
    private void handleMinOccupancyPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreePage page)
            throws DbException, IOException, TransactionAbortedException {//半满处理
        ...
        if(parentId.pgcateg() != BTreePageId.ROOT_PTR) {
            ...
            Iterator<BTreeEntry> ite = parent.iterator();//得到parent的entries
            while(ite.hasNext()) {
    ...
    public void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
        ...
        int stealNum = (page.getNumTuples()+sibling.getMaxTuples())/2-page.getNumTuples();
        if(stealNum<0) return ;
    ...
    public void stealFromLeftInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
        ...
        int stealNum = leftSibling.getNumEntries()-(leftSibling.getNumEntries()-page.getNumEntries())/2;
        if(stealNum<=0)
            return ;
    ...
    public void stealFromRightInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
        ...
        // the corresponding parent entry. Be sure to update the parent
        // pointers of all children in the entries that were moved.
        int stealNum = rightSibling.getNumEntries()-(rightSibling.getNumEntries()-page.getNumEntries())/2;
        if(stealNum<0)
            return;
    ...
    public void mergeInternalPages(TransactionId tid, Map<PageId, Page> dirtypages,
        ...
        parent.deleteKeyAndLeftChild(midEntry);
        leftPage.insertEntry(rightFirstEntry);
        while(rightIterator.hasNext()){
            rightFirstEntry = rightIterator.next();
            leftPage.insertEntry(rightFirstEntry);
            rightPage.deleteKeyAndRightChild(rightFirstEntry);
        }
...
//因为删除页是逻辑删除。所以物理上的内容在创建新页之后，记得把新页替代可能存在的老页
