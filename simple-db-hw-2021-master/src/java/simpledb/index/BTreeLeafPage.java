...
public class BTreeLeafPage extends BTreePage {
	...
	private int getHeaderSize() {        
		...
		int hb = (tuplesPerPage / 8);//一个字节对应八个tuple
		if (hb * 8 < tuplesPerPage) hb++;//可能有不满的情况，需+1
	...
	public byte[] getPageData() {
		...
		ByteArrayOutputStream baos = new ByteArrayOutputStream(len);//创建一个字节数组缓冲区
		DataOutputStream dos = new DataOutputStream(baos);
//后续对 dos 进行的写操作，实际上是将数据写入到 baos 中。
	...
	public void insertTuple(Tuple t) throws DbException {
		...
		int goodSlot = -1;
		if(emptySlot < lessOrEqKey) {//待插入的slot左侧有空的slot，左移
			...
			goodSlot = lessOrEqKey + 1;//记录最后新tuple在哪
		}
